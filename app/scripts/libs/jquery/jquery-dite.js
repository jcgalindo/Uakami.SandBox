...
*https://analytics.moontoast.com/track.gif?http_host=infiniti.moontoast.com&server_protocol=https&service_type=swf&catalog_id=2632&cart_id=&request_uri=/engagement_app/InfinityEngagementVideoLiveReveal/2632/default/videoAutoPlay/value/FF3nZLXOj5c/
        ...
        */
        function pixelTrack (data) {
            ...
            */
            ...
            var $img, url = '//analytics.moontoast.com/track.gif?http_host='+diteConfig.originDomain+'&request_uri=/'+diteConfig.appType+'/'+ diteConfig.appDiteName + '/' + diteConfig.catId + '/default/' + eventType +'&referrer='+diteConfig.referrer+'&user_agent='+diteConfig.userAgent+'&accept_language='+diteConfig.accept_language+'&request_method=get&server_protocol='+diteConfig.protocol+'&service_type=js&tracking_token='+diteConfig.trackingToken+'&origin_domain='+diteConfig.originDomain+'&catalog_id='+diteConfig.catId;

            /*
            var $img, url = '//analytics.moontoast.com/track.gif?appType=[' + diteConfig.appType + ']&environment=[' + diteConfig.environment + ']&offerID=[' + diteConfig.offerID + ']&screenName=[' + data.screenName + ']&eventType=[' + (data.eventType || '') +']&eventName=[' + (data.eventName || '') + ']&eventData=[' + (data.eventData || '') + ']&originDomain=[' + (diteConfig.originDomain) + ']&timeZone=[' + getTimezoneName() + ']&trackingToken=[' + diteConfig.trackingToken + ']&productionState=[' + diteConfig.productionState + ']';
            */
            $img = new Image();
            $img.src = url;
        ...
        */
        $.diteSetup = function (data) {
            diteConfig = $.extend(diteConfig, data);
        };
        ...
        * @param {Object} data
        */
        $('body').bind('dite', function (event, data) {
            $.dite(data);
        });
        ...
        * @param {Object} data
        */
        $('body').bind('dite-setup', function (event, data) {
            $.diteSetup(data);
        });
